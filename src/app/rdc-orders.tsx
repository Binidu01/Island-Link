...
export default function RDCOrders() {
  ...
  const calculateStatusStats = (ordersData: Order[]) => {
    ...
    ordersData.forEach((order) => {
      ...
      if (status === 'canceled') {
        stats.cancelled++
      } else if (stats[status as keyof StatusStats] !== undefined) {
  ...
  useEffect(() => {
    ...
    if (filters.status !== 'all') {
      if (filters.status === 'cancelled') {
        // Match both 'cancelled' and 'canceled' (US spelling) for safety
  ...
  // ⭐ NEW: Restore product stocks when an order is cancelled
  const restoreProductStocks = async (order: Order) => {
    try {
      console.log('Restoring product stocks for cancelled order:', order.id)
      ...
      for (const item of order.items) {
        ...
        try {
          ...
          if (productDoc.exists()) {
            ...
            console.log(`Restored stock for ${item.productId}: ${currentStock} -> ${newStock}`)
          }
        } catch (productError) {
      ...
      if (stockUpdates.length > 0) {
        await logAuditAction(
          ...
          `Restored stocks for cancelled order ${getOrderNumber(order)}. Changes: ${JSON.stringify(stockUpdates)}`,
          order.id
        )
      }
    } catch (error) {
      console.error('Error restoring product stocks:', error)
      throw error
    }
  }
    ...
    try {
      ...
      switch (newStatus.toLowerCase()) {
        ...
        case 'canceled':
          ...
          statusIcon = '❌'
          additionalInfo = `
            <p style="color: #6b7280; margin: 10px 0;">Cancelled by: <strong>${rdcLocation}</strong></p>
            ${note ? `<p style="color: #6b7280; margin: 10px 0;">Reason: <strong>${note}</strong></p>` : ''}
            ${
...
© ${new Date().getFullYear()} IslandLink Smart Distribution. All rights reserved.
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      // For online payments, mark as needing refund
      if (selectedOrder.paymentMethod?.toLowerCase() !== 'cod' && selectedOrder.pay === 'paid') {
        updateData.pay = 'refunded'
      }

      // If stock was already reduced (order was confirmed/processing), restore it
      if (
        ['confirmed', 'processing'].includes(selectedOrder.status?.toLowerCase() || '')
      ) {
        await restoreProductStocks(selectedOrder)
      }
      ...
      await logAuditAction(
        'Cancel Order (RDC)',
        `Order ${getOrderNumber(selectedOrder)} cancelled by RDC Staff at ${rdcLocation}. Reason: ${cancellationReason.trim()}`,
        selectedOrder.id
      )
      ...
      showSuccess(
        `Order ${getOrderNumber(selectedOrder)} has been cancelled successfully!${selectedOrder.paymentMethod?.toLowerCase() !== 'cod' && selectedOrder.pay === 'paid' ? ' Refund will be processed.' : ''}`
      )
    } catch (error: any) {
  ...
  ): { value: string; label: string; noteRequired?: boolean; isDestructive?: boolean }[] => {
    const statusFlow: Record<string, { value: string; label: string; noteRequired?: boolean; isDestructive?: boolean }[]> = {
      pending: [
        { value: 'confirmed', label: 'Confirm Order' },
        { value: 'cancelled', label: 'Cancel Order', noteRequired: true, isDestructive: true },
      ],
      confirmed: [
      ...
      processing: [
        // RDC cannot cancel once it's handed off to logistics
      ],
      cancelled: [
  ...
  const canRDCStaffUpdate = (order: Order) => {
    ...
    // Only allow update for pending/confirmed. Cancelled orders show no actions.
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 via-blue-50 to-indigo-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <p className="text-sm text-gray-600 mt-4">
            RDC staff can update: Pending → Confirmed → Processing, or cancel Pending/Confirmed orders
          </p>
        </div>
                    ...
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2 text-sm">
                          ...
                          {isCancelled && (
                            <span className="text-red-400" title="Cancelled order">
                              Cancelled
                            </span>
                          )}
                        </div>
              ...
              <div className="p-6 space-y-6">
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    ...
                    {selectedOrder.cancelledAt && (
                      <p className="text-xs text-red-600 mt-1">
                        Cancelled on {formatTimestamp(selectedOrder.cancelledAt)}
                      </p>
                    )}
                  </div>
                )}
                ...
                {canRDCStaffUpdate(selectedOrder) && (
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 bg-white border-2 border-red-300 text-red-600 rounded-xl hover:bg-red-50 transition font-semibold"
                    >
                      Cancel Order
                    </button>
                  </div>
                ...
                {selectedOrder.status?.toLowerCase() === 'processing' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
                    <p className="text-blue-700 font-medium">📦 Order is processing at RDC</p>
                    <p className="text-blue-600 text-sm mt-1">Ready for Logistics team pickup</p>
                  </div>
                )}
                {selectedOrder.status?.toLowerCase() === 'delivered' && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                    <p className="text-green-700 font-medium">✅ Order delivered by Logistics team</p>
                    <p className="text-green-600 text-sm mt-1">Delivery completed by Logistics team</p>
                  </div>
                )}
                {selectedOrder.status?.toLowerCase() === 'rejected' && (
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                    <p className="text-red-700 font-medium">⚠️ Order rejected by Logistics team</p>
                    <p className="text-red-600 text-sm mt-1">Rejection handled by Logistics team</p>
                  </div>
                )}
                {(selectedOrder.status?.toLowerCase() === 'cancelled' ||
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                    ...
                    <p className="text-red-600 text-sm mt-1">
                      {selectedOrder.cancellationReason || 'Order has been cancelled'}
                    </p>
                  </div>
                )}
              </div>
                  ...
                  <div className="space-y-3">
                    {getStatusOptions(selectedOrder.status || 'pending').map((option) => (
                      <button
                        ...
                        onClick={() => {
                          if (option.value === 'cancelled') {
                            ...
                            handleStatusConfirm(option.value)
                          }
                        }}
                        disabled={updatingStatus}
                        className={`w-full px-4 py-2.5 border rounded-xl transition font-medium disabled:opacity-50 flex items-center justify-between ${
                      ...
                      >
                        ...
                        <span
                          className={`text-xs ${option.isDestructive ? 'text-red-500' : 'text-gray-500'}`}
                        >
      ...
      {showCancelModal && selectedOrder && (
        <>
          <div
            className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50"
            onClick={() => !updatingStatus && setShowCancelModal(false)}
          />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
              <div className="p-6 border-b flex justify-between items-center">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-900">{getCustomerName(selectedOrder)}</p>
                </div>

                {selectedOrder.paymentMethod?.toLowerCase() !== 'cod' &&
                  selectedOrder.pay === 'paid' && (
              ...
              <div className="p-6 border-t flex gap-3">
                ...
                <button
                  ...
                  disabled={updatingStatus || !cancellationReason.trim()}
